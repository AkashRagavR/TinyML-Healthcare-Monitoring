// ============================================================
//  ARDUINO NANO 33 BLE - Integrated Health Monitor (Pure AI Mode)
...
constexpr int tensor_arena_size = 32 * 1024; // Reduced for Nano 33 BLE (256KB RAM)
...
void loop() {
  ...
  if (now - lastI2CRead >= I2C_READ_MS) {
    ...
    if (Wire.endTransmission(false) == 0) {
      // Cast to uint8_t for Nano 33 BLE Wire API compatibility
      if (Wire.requestFrom((uint8_t)MAX30102_ADDR, (uint8_t)6) == 6) {
    ...
    if (Wire.endTransmission(false) == 0) {
      // Cast to uint8_t for Nano 33 BLE Wire API compatibility
      if (Wire.requestFrom((uint8_t)MLX90614_ADDR, (uint8_t)3) == 3) {
